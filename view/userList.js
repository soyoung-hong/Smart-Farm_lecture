module.exports.userList = function(rows) {
    ...
    for (row of rows) {
        users += '<tr>';
        users += `<td>${row.uid}</td>
                    <td><a href="${row.name}.html">${row.name}</a>
                    </td><td>${row.deptName}</td>
                    <td>${row.tel}</td><td>${row.ts}</td>
                    <td><a href="/user/update/uid/${row.uid}"><i class="fas fa-edit"></i></a>&nbsp;
                        <a href="/user/password/uid/${row.uid}"><i class="fas fa-key"></i></a>&nbsp;
                        <a href="/user/delete/uid/${row.uid}"><i class="fas fa-trash-alt"></i></a></td>`;
        users += '</tr>';
    }
    return `
<!DOCTYPE html>
...
<body>
    <div class="container">
        <nav class="navbar navbar-expand-lg navbar-light bg-light">
            <img src="dog2.jpg" class="d-inline-block align-top" alt="">&nbsp;&nbsp;&nbsp;
